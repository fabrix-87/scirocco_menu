...
export default function MenuCard({ item, lang }: MenuCardProps) {
  ...
  return (
    <>
      {/* CARD PRINCIPALE */}
      <div className="group bg-scirocco-card rounded-xl overflow-hidden border border-scirocco-line shadow-sm flex h-40 transition-all duration-300 ease-out hover:-translate-y-1 hover:border-scirocco-gold/60 hover:shadow-md">
        {/* Immagine in formato Quadrato Perfetto maggiorato con cursore zoom-in */}
        <div
          onClick={() => item.image && setIsZoomed(true)}
          className={`w-38 h-40 shrink-0 bg-scirocco-surface flex items-center justify-center border-r border-scirocco-line relative overflow-hidden ${item.image ? "cursor-zoom-in" : ""}`}
        >
          {item.image ? (
            <Image
            ...
            <Image
              src={`icona-scirocco.svg`}
              className="text-scirocco-muted transition-transform duration-300 group-hover:scale-110"
              alt={"Scirocco 20.25"}
              width={80}
              height={80}
            />
